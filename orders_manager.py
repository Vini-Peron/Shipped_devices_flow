...
logging.basicConfig(filename='data/sn_logger.log', filemode='a', format='%(asctime)s %(message)s', level=logging.INFO)
...
def collect_serial_numbers(raw_order_data):
    ...
    for order_num in raw_order_data:
        try:
            ...
            device_checking_list[device_order] = {'S/Ns': []}  #TODO turn into a dict to merge serials and emails
            ...
            print("S/N not assigned yet.")

    print(device_checking_list) 
    order_serial_dict = {}
...
if __name__ == "__main__":
    ...
    file_path = 'data/AllOrdersDCL28062022.csv'  #TODO input / argparse / fetch file directly via DCL API
    ...
    # Complete cancelled orders - this likely broke because of email_dict merge which has been removed.
    # cancelled_orders = new_orders_df[new_orders_df['Stage Description'] == 'Order Cancelled']
    # cancelled_orders = cancelled_orders[['Order #']].to_dict()
    # manage_orders_list(cancelled_orders, cancel_ord=True)  # This function call breaks the data_dump function semantically
    #TODO check order date is not older than 4 days ALERT
